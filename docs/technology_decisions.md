...
The guardrails service still exposes a single `POST /check/input` endpoint for both stages (rather than two separate endpoints) — the caller (an n8n node) indicates which stage it's calling for, and the service applies only the checks valid for that stage, given the data actually available at that point.
