...
- **Different consumers.** The RAG corpus is consumed by the RAG Service (converted to per-call documents for Amazon Bedrock Knowledge Base retrieval — see `services/rag_service/ingestion/`). The classifier dataset is consumed by the PyTorch training pipeline (feature vectors + labels only).
